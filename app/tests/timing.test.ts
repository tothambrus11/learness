...
import { clip as made } from './make.js';

const clip = (engine: string, kind: Clip['kind'], genMs: number, audioMs: number): Clip =>
  made({ engine, kind, genMs, audioMs, backend: engine === 'supertonic' ? 'webgpu' : 'wasm' });
...
const clips = [
  ...
  /* an older clip, never timed */
  made({ engine: 'kokoro', kind: 'fr', genMs: 0, audioMs: 0 }),
];
...
test('real-time factor is generation over the audio it made', () => {
  ...
  assert.ok(Math.abs((fast?.rtf ?? 0) - (0.5 + 600 / 900) / 2) < 1e-9,
