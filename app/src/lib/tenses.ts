...
/* The group ids come from the pipeline: frcog/conjugation.py, SIMPLE_TENSES
   and COMPOUND_TENSES. */

/** The caption for one tense: what it is called, and what it is for. */
export interface TenseNote {
  /** The tense's name in French, as the table heads it: "Passé composé". */
  name: string;
  /** What the tense is for, in English, in a paragraph or two of prose. */
  use: string;
}

/** The caption for each tense, by group id — the ids a `ConjugationGroup`
 *  carries, plus the compound ones ("pc", "pqp") the table builds itself. A
 *  tense with no note here is shown with its id and no prose, so a group the
 *  catalogue adds cannot break the popover. */
export const TENSE_NOTES: Record<string, TenseNote> = {
  pres: {
    name: 'Présent',
    use:
      'What is happening now, what is generally true, and what you do habitually. ' +
  ...
  imp: {
    name: 'Imparfait',
    use:
      'The past as a scene rather than an event: what was going on, what used to happen, ' +
  ...
  fut: {
    name: 'Futur simple',
    use:
      'What will happen: je partirai, "I will leave". Speech often prefers aller + infinitive ' +
  ...
  cond: {
    name: 'Conditionnel',
    use:
      'What would happen: je partirais, "I would leave", usually with an if-clause in the ' +
  ...
  subj: {
    name: 'Subjonctif',
    use:
      'Not a time but a mood. It follows que after verbs and phrases of wanting, needing, ' +
  ...
  imper: {
    name: 'Impératif',
    use:
      'Orders, requests and suggestions with no pronoun: mange, "eat"; allons-y, "let’s go"; ' +
  ...
  hist: {
    name: 'Passé simple',
    use:
      'The past of narrative writing: novels, history, newspapers. Il partit, "he left". ' +
  ...
  subjimp: {
    ...
    use:
      'A literary tense that follows que in the past, where spoken French uses the ' +
  ...
  pc: {
    name: 'Passé composé',
    use:
      'The everyday past for things that happened: j’ai mangé, "I ate" or "I have eaten". ' +
  ...
  pqp: {
    ...
    use:
      'Something that had already happened before another past event: j’avais mangé quand ' +
      'il est arrivé, "I had eaten when he arrived". Auxiliary in the imparfait.',
  },
  futant: {
    name: 'Futur antérieur',
    use:
      'Something that will have happened by a point in the future: j’aurai fini avant midi, ' +
  ...
  condp: {
    ...
    use:
      'Something that would have happened: j’aurais aimé venir, "I would have liked to come", ' +
  ...
  subjp: {
    ...
    use:
      'The subjonctif for something already completed: je suis content que tu sois venu, ' +
