...
import type { TransferPolicy } from './model.js';
import { MINUTE_MS, nowMs } from './units.js';
import type { Millis } from './units.js';

export const POLICIES: readonly TransferPolicy[] = ['off', 'unmetered', 'always'];
export const DEFAULT_POLICY: TransferPolicy = 'always';        // sync: small and frequent
export const DEFAULT_BULK_POLICY: TransferPolicy = 'unmetered'; // audio: large, occasional

/** Whether to sync right now, and the sentence to show when not. */
export interface SyncVerdict { sync: boolean; reason: string }
...
export function shouldAutoSync({
  ...
  online = true,
  configured = false,
  lastSyncAt = 0 as Millis,
  now = nowMs(),
  minIntervalMs = 15 * MINUTE_MS,
  busy = false,
}: {
  ...
  if (lastSyncAt && since < minIntervalMs) {
    return no(`synced ${Math.round(since / MINUTE_MS)} min ago`);
  }
  return { sync: true, reason: 'due for an automatic sync' };
}

const no = (reason: string): SyncVerdict => ({ sync: false, reason });
...
/** May a transfer go ahead: yes, no, or ask the person first. `urgent` marks
 *  the case where the connection is probably theirs to pay for. */
